...
function allSlider(){
	...
	// custom pager
	$('.switchers').on('click', '.slide-prev', function(e){
		$(this).parents('.main-slider').find('.bx-prev').trigger('click');
		e.preventDefault();
	});

	$('.switchers').on('click', '.slide-next', function(e){
		$(this).parents('.main-slider').find('.bx-next').trigger('click');
		e.preventDefault();
	});
...
/*navScroller init*/
function navScrollerInit(){
	$('.nav-list>li>a').navScroller();
}
/*navScroller init end*/
